...
///////////My Electron Variables//////////////////////////////
   ...
   //Genparticles Variables
     ...
     std::vector<double> gen_Vx_;
     std::vector<double> gen_Vy_;
     std::vector<double> gen_Vz_;
     std::vector<double> gen_charge_;
     std::vector<int> gen_mom_;
     std::vector<int> gen_daughter_;
     std::vector<int> gen_daughter_pdgid_;
     std::vector<int> gen_mom_pdgid_; 
     std::vector<float>   gen_mcE;
     std::vector<float>   gen_mcEt;
...
////////////////////Defining variables to hold the properties of dressed electrons///////////////////////////
     ...
     std::vector<double> Dressed_Ele_charge;
...
/////////////Z peak///////////////////////////////////////
      edm::Handle<std::vector<reco::GenParticle> > gpchandle; 
     //                                                 change by me// edm::Handle<reco::GenParticleCollection> gpchandle;
/////////////////Jets///////////////////////////////
      edm::Handle<reco::GenJetCollection> gjchandle;
      edm::Handle<std::vector<reco::PFJet>> recjhandle;
...
//      edm::Handle<edm::ValueMap<bool> > tight_id_decisions;
    ...
    edm::EDGetTokenT<EcalRecHitCollection> recHitCollectionEBToken_;
      ...
      edm::EDGetTokenT<std::vector<reco::GenParticle> > gptoken_;
      edm::EDGetTokenT<reco::GenJetCollection> gjctoken_;
      edm::EDGetTokenT<std::vector<reco::PFJet>> recjtoken_;
...
ZEE_RecHit_NTuplizer::ZEE_RecHit_NTuplizer(const edm::ParameterSet& iConfig):
   ...
   xtoken_(consumes<GenEventInfoProduct>(edm::InputTag("generator"))),
	//gpctoken_(consumes<reco::GenParticleCollection>(edm::InputTag("GenParticles"))),
        gjctoken_(consumes<reco::GenJetCollection>(edm::InputTag("ak8GenJets"))),
        recjtoken_(consumes<std::vector<reco::PFJet>>(edm::InputTag("ak8PFJetsCHS")))
        //jets_(consumes<pat::Jet>(edm::InputTag("slimmedJetsAK8")))vector<reco::PFJet> 
{   
   //now do what ever initialization is needed
   isMC_= iConfig.getParameter<bool>("isMC");
gptoken_ = mayConsume<std::vector<reco::GenParticle>> (iConfig.getParameter<edm::InputTag>("genParticles"));
...
/*   iEvent.getByToken(HFElectronsToken_, HFElectrons);
   ...
   if(isMC_){
   	...
   	iEvent.getByToken(gptoken_, gpchandle);
	                          //iEvent.getByToken(gpctoken_,gpchandle);
        iEvent.getByToken(gjctoken_, gjchandle);
  //      std::cout<<"succesfullyly i event part done!! "<<std::endl;
/*
...
std::cout<<"gpchandle:: "<<gpchandle->size()<<std::endl;
  */
     /*
   	qS=xhandle->qScale();
...
const reco::GenParticle* z_boson=nullptr;
   ...
   int genIndex = 0;
//////////////////////////
//std::cout<<"beforing entering thwe main isMC_"<<std::endl;   
if (isMC_){
//std::cout<<"insid first mc cond"<<std::endl;
	/*for(edm::View<GenParticle>::const_iterator part = gpchandle->begin(); part!=gpchandle->end(); ++part){
...
////////////Lets grab a particle////////////////////////
        ...
        for (unsigned int i = 0; i <gpchandle->size(); ++i){
    //          std::cout<<"*************************We are inside the gpc handle loop"<<endl;
              const reco::GenParticle* gpart = &gpchandle->at(i);
              if(gpart ->status()==1 && abs(gpart ->pdgId())==11){
			Ele_Gen_Pt.push_back(gpart->pt());
			Ele_Gen_Eta.push_back(gpart->eta());
			Ele_Gen_Phi.push_back(gpart->phi());
			Ele_Gen_E.push_back(gpart->energy());
			}
                               ...
                               ///Getting the corresponding naked electron
                                ...
                                //If we have born and naked electron we can find the dressed electron
                                        ...
                                        if (bornEle_0 && nakedEle_0){
                                                ...
                                                dressedEle_0=GetDressedLepton(bornEle_0, nakedEle_0, 0.4, 11);
                             ...
                             //           cout<<"The current energy for de0 is "<<(dressedEle_0->energy())<<endl;
                                ...
                                /////If we already found the first born electron then lets find and do the samething for the second born electron
                                        ...
                                        if (bornEle_1 && nakedEle_1){
                                                ...
                                                dressedEle_1=GetDressedLepton(bornEle_1, nakedEle_1, 0.4, 11);
        ...
        if(dressedEle_0 !=nullptr && dressedEle_1 !=nullptr){
                if (dressedEle_0->pt() < dressedEle_1->pt()){
                        std::cout<<"Swapping active"<<std::endl;
                ...
                Dressed_Ele_charge.push_back(dressedEle_0->charge());
                ...
                Dressed_Ele_charge.push_back(dressedEle_1->charge());
                ...
                for (size_t l = 0; l<electrons->size(); ++l){
                        ...
                        std::cout<<"electrons fine"<<std::endl;
                        if (deltaR(dressedEle_0->eta(), dressedEle_0->phi(), ele1->eta(), ele1->phi()) <0.4 || deltaR(dressedEle_1->eta(), dressedEle_1->phi(), ele1->eta(), ele1->phi()) < 0.4){
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////
  ...
  for (std::vector<reco::GenParticle>::const_iterator ip = gpchandle->begin(); ip != gpchandle->end(); ++ip) {
  ...
  //  int status = ip->status();
      ...
      pdgId_.push_back(p->pdgId());
      gen_Vx_.push_back(p->vx());
      gen_Vy_.push_back(p->vy());
      gen_Vz_.push_back(p->vz());
      gen_pt_.push_back(p->pt());
      gen_mass_.push_back(p->mass());
      gen_eta_.push_back(p->eta());
      gen_phi_.push_back(p->phi());
      gen_mcE.push_back(p->energy());
      gen_mcEt.push_back(p->et());
      gen_status_.push_back(p->status());
      gen_charge_.push_back(p->charge());
...
void ZEE_RecHit_NTuplizer::beginJob()
{
	...
	T->Branch("HFEMClust_eta", &HFEMClust_eta_);
        ...
        T->Branch("pdgId", &pdgId_);
        T->Branch("gen_status", &gen_status_);
        T->Branch("gen_pt", &gen_pt_);
        T->Branch("gen_eta", &gen_eta_);
        T->Branch("gen_phi", &gen_phi_);
        T->Branch("gen_mass", &gen_mass_);
        T->Branch("gen_Vx", &gen_Vx_);
        T->Branch("gen_Vy", &gen_Vy_);
        T->Branch("gen_Vz", &gen_Vz_);
        T->Branch("gen_charge", &gen_charge_);
	T->Branch("gen_mcE", &gen_mcE);
	T->Branch("gen_mcEt", &gen_mcEt);
...
/////////////////////////////////////////////////////////
	if(isMC_){
		...
		T->Branch("Ele_Gen_Pt" , &Ele_Gen_Pt);
                ...
                T->Branch("DressedEle_Charge", &Dressed_Ele_charge);
...
void ZEE_RecHit_NTuplizer::ClearTreeVectors()
{
	...
	pdgId_.clear();
        ...
        gen_mom_.clear();
        gen_daughter_.clear();
        gen_daughter_pdgid_.clear();
        gen_mom_pdgid_.clear();
	gen_mcE.clear();
	gen_mcEt.clear();
